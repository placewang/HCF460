  ...
  MBHOST *phost=&mbHost[idx];
	...
	switch(phost->state)
	{
                ...
                if(configend_sendonline==0)//配置id阶段
                 {
                  if((phost->txBuf[3]==CONFIGCMD)&&(phost->txBuf[4]==CONFIGCMD_ASK))//查询ID配置成功的命令
                   {
                     config_failflag++;
                     if(config_failflag>=2*MBH_ERR_MAX_TIMES)
		...
		case MBH_STATE_TIMES_ERR:
         if(((phost->txBuf[3]==CMDUART_COM_CHECKCONFIG))||(phost->txBuf[3]==CMDUART_COM_CONFIGPARA)) 
            {
            }
         else              
            Alarmlist_Push(&alarmlist,phost->txBuf[0],ERR_SONBOARD_UNLINK,0);
